...
class AccessUserListController {
	...
	private function query(){
		...
		$this->sql .= " ,a1.cl_name ";
		$this->sql .= " ,a1.cl_email ";
		$this->sql .= " ,a1.cl_pwd "; 
		$this->sql .= " ,a1.cl_date_insert ";
		$this->sql .= " ,a1.cl_date_update ";
		$this->sql .= " FROM tb_access_user AS a1";
	 ...
	 private function dataGrid() {
		...
		if(!$result){
			...
			if(!empty($_SESSION ["REDIRECTION"])){
				...
				$dg->column->linkUrl ( "formAccessUser.php" );
			}
		}
		...
		$dg->column->start( );
		$dg->column->refDataBaseTableColumn( "cl_email" );
		$dg->column->title( "E-MAIL" );
		$dg->column->align( "left" );
		$dg->column->width(  );

		$dg->column->start( );
		$dg->column->refDataBaseTableColumn( "cl_pwd" );
		$dg->column->title( "PASSWORD" );
		$dg->column->align( "left" );
		$dg->column->width(  );
