...
class AccessProfileListController {
	...
	private function query(){
		...
		$this->sql .= " FROM tb_access_page AS a1 " ;
	 ...
	 private function dataGrid() {
		...
		if(!$result){
			...
			if(!empty($_SESSION ["REDIRECTION"])){
				...
				$dg->column->linkUrl ( "formAccessPage.php" );
			}
		}
		...
		$dg->column->align( "left" );//Posição da informação na coluna
		$dg->column->width(  );//Define a lagura da coluna 
